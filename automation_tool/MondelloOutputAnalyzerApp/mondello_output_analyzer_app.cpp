...
int main(int argc, char *argv[])
{
    ...
    char *RawDumpPath = argv[2];
    int RawDumpFrameCount = atoi(argv[3]);
    MEDIA_FORMAT RawDumpFormat = GetFormatFromString(argv[4]);
    int ImageW = atoi(argv[5]);
    int ImageH = atoi(argv[6]);
    char *ReferenceImagePath = argv[7];
    bool Alignment = atoi(argv[8]);
    int result=0;
    ...
    if (!Loadinputimage(RawDumpPath, RawDumpFrameCount, ImageW, ImageH, RawDumpFormat, RawDumpImages, Alignment))
    {
    ...
    int start_frame=1;
    if (strstr(RawDumpPath,"weaving") != NULL || strstr(RawDumpPath,"WEAVING") != NULL)
    {
        start_frame=2;
    }

    if ( strstr(RawDumpPath,"SCALING") != NULL && strstr(RawDumpPath,"Interlaced") != NULL && strstr(RawDumpPath, "NV12") != NULL)
    {
        LOG_INFO("The scaling weaving case  skip three frame",NULL);
        start_frame = 3;
    }
    for (int i = start_frame; i < RawDumpFrameCount; i++)
    {
        INPUT_IMAGE img1;
        INPUT_IMAGE img2;
