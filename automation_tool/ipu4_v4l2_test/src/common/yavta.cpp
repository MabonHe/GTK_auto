...
static void video_enum_formats(struct device *dev, enum v4l2_buf_type type)
{
    ...
    for (i = 0; ; ++i) {
        ...
        printf("\tType: %s (%u)\n", v4l2_buf_type_name((enum v4l2_buf_type)fmt.type),
            fmt.type);
        printf("\tName: %.32s\n", fmt.description);
...
int yavta_main(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "B:c::Cd:f:F::hi:Iln:pq:r:R::s:t:uw:", opts, NULL)) != -1) {
        ...
        case 'B':
            ...
            video_set_buf_type(&dev, (enum v4l2_buf_type)ret);
            break;
        case 'c':
            do_capture = 1;
            if (optarg)
            ...
            break;
        case 'C':
            fill_mode = (enum buffer_fill_mode)((int)fill_mode | BUFFER_FILL_PADDING);
            break;
        case 'd':
            delay = atoi(optarg);
            break;
        ...
        case 'i':
            ...
            break;
        case 'I':
            fill_mode = (enum buffer_fill_mode) ((int)fill_mode | BUFFER_FILL_FRAME);
            break;
        case 'l':
            do_list_controls = 1;
            break;
